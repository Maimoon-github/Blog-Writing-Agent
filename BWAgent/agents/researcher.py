...
async def researcher_node(state: GraphState) -> Dict[str, Any]:
    ...
    try:
        ...
        if cached is not None:
            ...
            # cached is expected to be a dict with "summary" and "source_urls"
    ...
    # 3. Perform web search (run in thread if search is synchronous)
...
async def _fetch_content_items(
    ...
    async def fetch_one(result: SearchResult) -> Dict[str, str]:
        """Fetch a single page using async_fetch_page_content (which is already async)."""
        try:
            # Use the async version of the scraper if available; otherwise fallback to to_thread
            # Here we assume async_fetch_page_content exists and is truly async.
            extracted_text = await async_fetch_page_content(result.url, max_chars=3000)
