...
# UPDATED: Import build_graph and state helpers instead of wiring inline
...
async def run_pipeline(initial_state: Dict[str, Any]) -> Dict[str, Any]:
    """Execute the graph with the given initial state and return final state."""
    # UPDATED: Build the graph (app and tracer) on each invocation
...
def main(topic: str, target_keyword: str = None, tone: str = "coach", word_count_goal: int = 1500):
    """Synchronous entry point for CLI usage."""
    # UPDATED: Use make_initial_state to create the state
    initial_state = make_initial_state(
        topic=topic,
        target_keyword=target_keyword,
        tone=Tone(tone),               # convert string to enum
        word_count_goal=word_count_goal,
    )
    # ensure session_id is present (make_initial_state should include it)
