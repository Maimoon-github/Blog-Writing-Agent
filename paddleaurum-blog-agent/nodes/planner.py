...
async def planner_node(state: AgentState, *, llm: Ollama) -> dict:
    try:
        ...
        # Validate sub_queries count per prompt guidelines (3-5)
        if needs_research:
            if len(sub_queries) < 3:
                logger.warning(
                    "Planner produced only %d sub-queries (expected 3-5). "
                    "Consider refining prompts or adding fallback queries.",
                    len(sub_queries)
                )
                # Optionally pad with generic queries? Not adding to avoid irrelevant results.
            elif len(sub_queries) > 5:
                logger.warning(
                    "Planner produced %d sub-queries, truncating to first 5.",
                    len(sub_queries)
                )
