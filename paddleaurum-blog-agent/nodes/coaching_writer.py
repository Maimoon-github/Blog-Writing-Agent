...
async def coaching_writer_node(state: AgentState, *, llm, vectordb) -> dict:
    try:
        topic: str = state["topic"]
        tone: Tone = state.get("tone") or Tone.COACH
        word_count_goal: int = state.get("word_count_goal", 1500)
        ...
        # Run the synchronous CrewAI agent in a thread pool to avoid blocking the event loop
        loop = asyncio.get_running_loop()
        draft: str = await loop.run_in_executor(
            None,
            run_coach_writer,
            llm,
            vectordb,
