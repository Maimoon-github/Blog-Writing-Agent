...
#     """
#     Fetch web page content synchronously and extract clean text.
...
#             logger.info("fetch_page_content_success", url=url, chars=len(text_content))
#             return text_content
...
#                 logger.warning("fetch_page_content_status_error_retry", url=url, status_code=e.response.status_code, attempt=attempt + 1)
#                 time.sleep(BACKOFF_FACTOR ** attempt)
#             else:
#                 logger.error("fetch_page_content_permanent_error", url=url, error=str(e), status_code=e.response.status_code)
#                 return ""
#         except httpx.RequestError as e:
#             logger.warning("fetch_page_content_network_error_retry", url=url, attempt=attempt + 1, error=str(e))
#             if attempt < MAX_RETRIES - 1:
#                 time.sleep(BACKOFF_FACTOR ** attempt)
#         except Exception as e:
#             logger.error("fetch_page_content_unexpected_error", url=url, error=str(e), exc_info=True)
#             return ""

#     logger.error("fetch_page_content_failed", url=url, error="max retries reached")
#     return ""
...
#             text_content = _process_html(response.text, max_chars)
#             logger.info("async_fetch_page_content_success", url=url, chars=len(text_content))
#             return text_content

#         except httpx.HTTPStatusError as e:
#             is_transient = e.response.status_code >= 500 or e.response.status_code == 429
#             if is_transient and attempt < MAX_RETRIES - 1:
#                 logger.warning("async_fetch_page_content_status_error_retry", url=url, status_code=e.response.status_code, attempt=attempt + 1)
#                 await asyncio.sleep(BACKOFF_FACTOR ** attempt)
#             else:
#                 logger.error("async_fetch_page_content_permanent_error", url=url, error=str(e), status_code=e.response.status_code)
#                 return ""
#         except httpx.RequestError as e:
#             logger.warning("async_fetch_page_content_network_error_retry", url=url, attempt=attempt + 1, error=str(e))
#             if attempt < MAX_RETRIES - 1:
#                 await asyncio.sleep(BACKOFF_FACTOR ** attempt)
#         except Exception as e:
#             logger.error("async_fetch_page_content_unexpected_error", url=url, error=str(e), exc_info=True)
#             return ""

#     logger.error("async_fetch_page_content_failed", url=url, error="max retries reached")
#     return ""
